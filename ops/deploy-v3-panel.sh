...
id=dates-v3-panel-$(date -u +%Y%m%dT%H%M%SZ)
stage=/home/root/.codex-staging/$id; rec=/home/root/.codex-backups/$id
localdir=.cache/$id
mkdir -p "$localdir"; chmod 700 "$localdir"
cp build/DatesPanel.qml "$localdir/"
...
cp ops/{repair-v3-panel,rollback-v3-panel}.sh "$localdir/"
for script in "$localdir/"*.sh; do bash -n "$script"; done
(cd "$localdir" && shasum -a 256 *.qml *.sh *.sha256 > SHA256SUMS)
...
ssh "${opts[@]}" "root@$host" "bash '$stage/repair-v3-panel.sh' prepare '$stage' '$reviewed'"
...
ssh "${opts[@]}" "root@$host" "systemd-run --unit=dates-v3-panel-install --collect --wait --pipe --property=RuntimeMaxSec=150 bash '$stage/repair-v3-panel.sh' activate '$stage' '$reviewed'"
