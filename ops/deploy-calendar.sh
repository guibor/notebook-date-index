#!/bin/bash
# Exact Pro v3 -> calendar payload, keeping the writer and all QMDs unchanged.
set -euo pipefail
cd "$(dirname "$0")/.."
...
id=dates-calendar-$(date -u +%Y%m%dT%H%M%SZ)
...
cp ops/{install-calendar,rollback-calendar}.sh "$localdir/"
...
ssh "${opts[@]}" "root@$host" "bash '$stage/install-calendar.sh' prepare '$stage' '$reviewed'"
...
ssh "${opts[@]}" "root@$host" "systemd-run --unit=dates-calendar-install --collect --wait --pipe --property=RuntimeMaxSec=150 bash '$stage/install-calendar.sh' activate '$stage' '$reviewed'"
