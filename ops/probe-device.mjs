...
const [host,notebook,target='pro']=process.argv.slice(2);
assert.ok(['pro','move'].includes(target));
...
assert.equal(fp,target==='pro' ? 'SHA256:dByHweKZkjDlZRBHdBisT5VD2kV85lClgtJExnDaTeE' : 'SHA256:osLWO+xA0s/qhzWV2jtGWAF6NlD/KEi/d6CEkt8MZMk');
...
assert.equal(remote('cat /sys/devices/soc0/machine').trim(),target==='pro' ? 'reMarkable Ferrari' : 'reMarkable Chiappa');
