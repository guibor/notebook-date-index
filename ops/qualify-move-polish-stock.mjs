// Same pinned harmless canary as r5, with a unique preverified transaction archive.
// Only its freshly captured stock PID/start literals change; xochitl is untouched.
import fs from 'node:fs';
import path from 'node:path';
import {execFileSync} from 'node:child_process';
import {createHash} from 'node:crypto';
import assert from 'node:assert/strict';
const [host,maintenance,id]=process.argv.slice(2);
assert.match(host||'',/^\d+\.\d+\.\d+\.\d+$/); assert.ok(maintenance);
assert.match(id||'',/^dates-move-polish-[0-9]{8}T[0-9]{6}Z-[0-9]+$/);
...
const rec='/home/root/.codex-backups/'+id;
...
for(const [old,value] of [['EXPECTED_STOCK_PID=5492',identity[0]],['EXPECTED_STOCK_START=10491381',identity[1]]]) {
 assert.equal(source['canary.sh'].split(old).length,2);
 source['canary.sh']=source['canary.sh'].replace(old,old.split('=')[0]+'='+value);
}
const local=path.resolve('.cache',id,'stock-canary'),upload='/run/'+id+'-stock-check';
...
const old='/run/remarkable-beta-os-systemd-shadow-canary',archive=rec+'/previous-canary';
remote(`set -eu
...
mv '${old}' '${archive}'; mkdir -m 700 '${upload}'`);
...
const result=remote(`systemd-run --collect --wait --pipe --unit=dates-move-polish-stock-check --property=Type=exec --property=KillMode=control-group --property=RuntimeMaxSec=90s --property=TimeoutStopSec=15s /usr/bin/env PROFILE='${upload}/profile.env' PROFILE_LIB='${upload}/profile-lib.sh' /bin/bash '${upload}/canary.sh'`);
fs.writeFileSync(local+'/result.log',result,{mode:0o600});
assert.match(result,/systemd_shadow_canary=passed/);
remote(`set -eu; test ! -e '${rec}/rolled-back'; touch '${rec}/canary-verified'; sync`);
console.log('move_polish_stock_check=passed stock_pid='+identity[0]+' evidence='+local);
